...
public:
    ...
    void computeKinematicsDownTheTree(const Eigen::VectorXd& gc){
        if (joint_.type== Joint::Type::FLOATING){
            joint_.jointPosition_W << gc(0), gc(1), gc(2);

            Eigen::Quaterniond b_q(gc(3),gc(4),gc(5),gc(6));
            joint_.rotation_W = b_q.normalized().toRotationMatrix();
        }
        else if(joint_.type== Joint::Type::REVOLUTE){
            ///gc = parent's position(3), orientation(3) & joint theta(1)
            joint_.jointRotations_P <<
            joint_.rotation_W <<
        }
    }
